...
var app = articulator.createServer(
  `${__dirname}/../example/api`,
  {
    faker,
    _: lodash,
...
describe("MATCH .../${method}.js", function() {
  describe("GET /express", function() {
    it("should return express route response", function(done) {
      request
        .get("/express?name=friend")
        .expect(200)
        .expect(function(res) {
          assert.equal(res.text, "express says hello friend");
        })
        .end(function(err, res) {
          if (err) return done(err);
          done();
